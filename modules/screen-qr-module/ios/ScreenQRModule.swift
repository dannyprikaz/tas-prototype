...
public class ScreenQRModule: Module {
    ...
    /// Returns the appropriate app group identifier based on the main app's bundle ID
    private var appGroupIdentifier: String {
        let bundleId = Bundle.main.bundleIdentifier ?? ""
        let appGroup = "group.\(bundleId)"
        NSLog("📦 ScreenQRModule using app group: \(appGroup)")
        return appGroup
    }
    
    /// Returns the appropriate broadcast extension identifier based on the main app's bundle ID
    private var broadcastExtensionIdentifier: String {
        let bundleId = Bundle.main.bundleIdentifier ?? ""
        let extensionId = "\(bundleId).TASBroadcastExtension"
        NSLog("📱 Using broadcast extension: \(extensionId)")
        return extensionId
    }
    ...
    public func definition() -> ModuleDefinition {
        ...
        AsyncFunction("startBroadcast") { () -> String in
            ...
            if let ud = UserDefaults(suiteName: self.appGroupIdentifier) {
                ud.set([], forKey: "lastDetectedQRSet")
                ud.set(true, forKey: "broadcastAttempted")
                NSLog("✅ Initialized app group data")
            } else {
                NSLog("❌ Failed to access app group: \(self.appGroupIdentifier)")
            }
    ...
    private func checkForDetectedQR() {
        NSLog("App entered foreground, checking for QR data in: \(self.appGroupIdentifier)")
